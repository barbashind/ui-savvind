...
const AccountingAccModalMobile = ({isOpen, setIsOpen} : TAccountingAccModalProps) => {
                ...
                >
                        <Layout direction="column" className={cnMixSpace({p: 's'})}>
                                <Layout direction="row" style={{justifyContent: 'space-between', alignItems:'center'}}>
                                        <Text view="brand" size="m" weight="semibold">Счета</Text>
                                ...
                                <Layout direction="row" className={cnMixSpace({mT:'m'})}>
                                        <Text style={{minWidth: '50px', maxWidth: '50px'}} className={cnMixSpace({ mL:'s'})} size="2xs" align="left" view="secondary">ID</Text>
                                        <Text style={{width: '100%'}} size="xs" align="left" view="secondary">Наименование</Text>
                                        <Text style={{minWidth: '70px', maxWidth: '70px'}} className={cnMixSpace({mR:'m'})} size="2xs" align="left" view="secondary">Объём (руб)</Text>
                                        <Text style={{minWidth: '50px', maxWidth: '50px'}} className={cnMixSpace({ mL:'s', mR:'m'})} size="2xs" align="left" view="secondary">Валюта</Text>
                                </Layout>
                                )}
                                <Layout direction="column">
                                        {!isLoading && user !== 'Matvei' && accounts?.map((acc : TAccount) => (
                                                <Layout direction="row" className={cnMixSpace({mT: 's', p:'s'})} style={{border: '1px solid rgba(0,65,102,.2)', borderRadius: '4px'}}>
                                                        <Text style={{minWidth: '50px', maxWidth: '50px'}} size="2xs">{acc?.accountId?.toString()}</Text>
                                                        <Text style={{width: '100%'}} size="2xs">{TextHighlith(acc?.name, searchText)}</Text>
                                                        <Text style={{minWidth: '70px', maxWidth: '70px'}} size="2xs">{acc.value?.toString()}</Text>
                                                        <Text style={{minWidth: '50px', maxWidth: '50px'}} size="2xs" className={cnMixSpace({mL: 's'})}>{acc?.currency}</Text>
                                                </Layout>
                                        ))}
...
export default AccountingAccModalMobile