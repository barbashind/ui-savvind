...
const PurchaseModal = ({isOpen, setIsOpen} : TPurchaseModalProps) => {
                ...
                >
                        ...
                        <Layout direction="column" style={{width: '100%'}} className={cnMixSpace({ p:'xl' })}>
                                <Layout direction="row" style={{justifyContent: 'space-between'}}>
                                        <Layout direction="row" >
                                                <Text size="xl" view="brand" style={{width: '100%'}} className={cnMixSpace({ mL:'m', mT: '2xs' })}>
                                                       {!data.batchId ?'Оформление разовой закупки' : 'Разовая закупка оформлена'}
                                                </Text>
                                        </Layout>
                                ...
                                <Layout direction="row" className={cnMixSpace({ mT:'m' })} >
                                        ...
                                        <Text size="s" style={{minWidth:'150px', maxWidth:'150px'}} className={cnMixSpace({ mR:'m' })} align="center">Сер. номер</Text>
                                        <Text size="s" style={{minWidth:'100px', maxWidth:'100px'}} className={cnMixSpace({ mR:'m' })} align="center">Количество</Text>
                                        <Text size="s" style={{minWidth:'150px', maxWidth:'150px'}} className={cnMixSpace({ mR:'m' })} >Цена закупки</Text>
                                        <Text size="s" style={{minWidth:'235px', maxWidth:'235px'}} className={cnMixSpace({ mR:'m' })} >Склад</Text>
                                ...
                                {!data?.batchId && (
                                        ...
                                        {purchases?.length > 0 && purchases?.filter(elem => (!elem.serialNumber)).map(itemCheck => (
                                                                <Layout key={purchases.indexOf(itemCheck).toString()} direction="row" className={cnMixSpace({ mT:'s' })}>
                                                                        <Text size="s" style={{minWidth:'15px', maxWidth:'15px'}} className={cnMixSpace({ mT:'l', mR:'m' })}>{(purchases.indexOf(itemCheck) + 1).toString()}</Text>
                                ...
                                <Layout direction="row" style={{justifyContent: 'space-between', alignItems: 'end'}}  flex={1} className={cnMixSpace({ mT:'l' })}>
                                        <Layout direction="row" style={{justifyContent: 'left'}}>
                                                        ...
                                                        <Text size="m" style={{minWidth: '110px'}} weight='semibold' view="brand" className={cnMixSpace({ mT:'2xs', mL:'xs' })}>{(purchases?.length > 0 ? purchases?.filter(el => (el.hasSerialNumber && el.serialNumber || !el.hasSerialNumber)).reduce((acc, item) => acc + (item.quantFinal ?? 1) * (item.costPriceAll ?? 0), 0).toString() : 0) + ' руб'}</Text>  
                                        </Layout>
